...
class newshealth_recent_post_widget extends WP_Widget {
	/* Widget setup */
	...
	function form( $instance ) {
		...
		$defaults = array(
			'title' => esc_html__('Bài viết mới nhất', 'newshealth'),
			'order' => 'DESC'
		);
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'select_cat' ) ); ?>">
				<?php esc_attr_e( 'Select Categories:', 'newshealth' ); ?>
            </label>
            ...
            <select id="<?php echo esc_attr( $this->get_field_id( 'select_cat' ) ); ?>"
                ...
                <option value="0" <?php echo( in_array( 0, $select_cat ) ? 'selected="selected"' : '' ); ?>>
					<?php esc_html_e( 'All Category', 'newshealth' ); ?>
                </option>
...
// Register widget
function newshealth_recent_post_widget_register() {
	register_widget( 'newshealth_recent_post_widget' );
}

add_action( 'widgets_init', 'newshealth_recent_post_widget_register' );